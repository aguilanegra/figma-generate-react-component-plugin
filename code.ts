...
figma.ui.onmessage = async (msg) => {
  if (msg.type === 'generate-component') {
    ...
    try {
      ...
      figma.ui.postMessage({
        type: 'error',
        message: error instanceof Error ? error.message : 'Failed to extract component data'
      });
    }
  }
...
function detectComponentType(node: ComponentNode | InstanceNode): string {
  ...
  // Default to generic component if type can't be determined
  return 'generic';
}

// Extract all relevant data from the component
async function extractComponentData(node: ComponentNode | InstanceNode, componentType: string): Promise<ComponentData> {
  ...
  if (node.type === 'TEXT') {
    ...
    if (node.lineHeight !== figma.mixed && typeof node.lineHeight === 'object') {
      ...
      if ('unit' in lineHeight && 'value' in lineHeight) {
        styles.lineHeight = `${lineHeight.value}${lineHeight.unit.toLowerCase()}`;
      }
    }
  }
...
async function extractStates(node: ComponentNode | InstanceNode): Promise<ComponentStateStyles> {
  ...
  if (Object.keys(states.hover).length === 0) {
    ...
    const baseStyles = extractStyles(node);
    if (baseStyles.backgroundColor && baseStyles.backgroundColor !== 'transparent') {
      // Darken background color slightly for hover
      states.hover.backgroundColor = darkenColor(baseStyles.backgroundColor, 0.1);
    }
  }
  ...
  if ('componentProperties' in node && node.componentProperties) {
    ...
    for (const key in node.componentProperties) {
      if (Object.prototype.hasOwnProperty.call(node.componentProperties, key)) {
        ...
        props[key] = {
          type: propType,
          defaultValue: typeof value.value === 'string' || typeof value.value === 'boolean' ? value.value : null
        };
      }
    }
  }
